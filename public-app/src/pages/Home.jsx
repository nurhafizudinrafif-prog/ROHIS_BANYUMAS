...
import { 
  ArrowRight, Users, School, Calendar, BookOpen, Sparkles, 
  Heart, MessageCircle, Library, FileText, ChevronRight,
  CheckCircle2, Clock, MapPin, Quote, ShieldCheck,
  UserCheck, Newspaper, Megaphone, Wallet
} from 'lucide-react';
import { extractDriveId, isDriveUrl } from '@shared/services/mediaHelper.js';

// Safe image URL resolver (Google Drive thumbnail + direct image)
...
function StatCard({ value, suffix = '+', label, delay = 0, colorClass = 'stat-green' }) {
  return (
...
function ArticleCard({ article, index }) {
  ...
  const coverUrl = getSafeMediaUrl(article.image || article.coverImage);

  return (
    <Link to={`/articles/${article.slug || article.id}`} className="card-editorial animate-fade-in-up"
      style={{ animationDelay: `${index * 100}ms`, textDecoration: 'none', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{
        height: 190,
      ...
      <div className="card-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '1.25rem' }}>
        <h3 style={{ color: 'var(--deep-pine)', fontSize: '1.05rem', lineHeight: 1.4, marginBottom: '0.5rem' }}>
          {article.title}
        </h3>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.78rem', color: 'rgba(13,43,34,0.5)' }}>
          <span>{article.author || 'Tim ROKABA'}</span>
          <span>•</span>
          <span>{new Date(article.publishedAt || article.date || Date.now()).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
        </div>
      </div>
    </Link>
  );
  ...
  return (
    ...
    >
      <div style={{
        ...
        <div style={{ fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.2rem', color: 'var(--antique-brass)' }}>
          {new Date(event.date).toLocaleDateString('id-ID', { month: 'short' }) || 'BLN'}
        </div>
      </div>
      <div style={{ flex: 1 }}>
        <span className="badge badge-gold" style={{ fontSize: '0.7rem', padding: '0.15rem 0.6rem', marginBottom: '0.4rem' }}>
          {event.category || 'Agenda'}
        </span>
        <h4 style={{ color: 'var(--deep-pine)', margin: '0.35rem 0 0.4rem', fontSize: '1.05rem', lineHeight: 1.3 }}>
          {event.title}
        </h4>
        <p style={{ fontSize: '0.82rem', color: 'rgba(13,43,34,0.6)', margin: 0, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          <MapPin size={14} style={{ color: 'var(--emerald)', flexShrink: 0 }} /> {event.location} {event.time ? `• 🕐 ${event.time}` : ''}
        </p>
      </div>
    </div>
  );
...
// 5 Division Default Data
const defaultDivisionPrograms = [
  {
    id: 1,
    division: 'SDM',
    ...
    programs: ['Latihan Dasar Kepemimpinan (LDK) ROHIS', 'Upgrading Skill Pengurus', 'Mentoring Kepemimpinan Pelajar'],
  },
  {
    id: 2,
    division: 'Dakwah',
    ...
    programs: ['Kajian Ahad Pagi Se-Banyumas', 'Safari Dakwah Sekolah', 'Tahsin Tilawah Al-Quran'],
  },
  {
    id: 3,
    ...
    programs: ['Produksi Konten Media Sosial', 'Penerbitan Buletin Digital', 'Workshop Desain & Videografi'],
  },
  {
    id: 4,
    division: 'HUMAS',
    ...
    programs: ['Silaturahmi Antar Sekolah', 'Kemitraan Kemenag & Lembaga Islam', 'Aksi Peduli Sosial & Bencana'],
  },
  {
    id: 5,
    division: 'DANUS',
    ...
    programs: ['Merchandise Resmi ROKABA', 'Bazaar Dakwah Kreatif', 'Edukasi Entrepreneur Syariah'],
  },
];
...
export default function Home() {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--deep-pine)' }}>
        <div className="animate-float" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'rgba(245,242,237,0.7)', fontFamily: 'var(--font-heading)' }}>Memuat data ekosistem dakwah...</p>
        </div>
      </div>
    );
  }

  // --- Dynamic CMS Data Mapping (Matching Admin 6 Tabs) ---
  const hero = home?.hero || {
    ...
    narrative: home?.welcome_msg || 'Wadah silaturahmi, kaderisasi kepemimpinan, dan sinergi dakwah pelajar Islam lintas SMA, SMK, dan MA se-Kabupaten Banyumas. Berakar pada akhlak, bergerak dalam karya nyata untuk generasi muda.',
  ...
  const timeline = Array.isArray(home?.timeline) && home.timeline.length > 0 ? home.timeline : [
    { id: 'tl-1', year: '2017', tag: 'AWAL JEJAK', title: 'Inisiasi & Silaturahmi Perdana', location: 'Purwokerto', desc: 'Pertemuan perdana perwakilan pengurus rohis sekolah se-Purwokerto yang melahirkan kesepakatan membentuk wadah silaturahmi terpadu.' },
    { id: 'tl-2', year: '2020', tag: 'KETAHANAN', title: 'Ruang Temu di Masa Pandemi', location: 'Banyumas', desc: 'Menghadapi masa karantina dengan menggelar kajian virtual berkala, mentoring daring, dan aksi kepedulian sosial bagi pelajar terdampak.' },
    { id: 'tl-3', year: '2022', tag: 'REKONSILIASI', title: 'Konsolidasi Akbar Pasca-Pandemi', location: 'Masjid Agung Baitussalam', desc: 'Kebangkitan tatap muka akbar, memperluas jejaring dakwah hingga ke SMA, SMK, dan MA di berbagai kecamatan Kabupaten Banyumas.' },
    { id: 'tl-4', year: '2024', tag: 'KURIKULUM', title: 'Standarisasi Modul Kaderisasi', location: 'Banyumas', desc: 'Penyusunan kurikulum Latihan Kepemimpinan ROHIS (LKRO) terstandar dan mentoring berkala demi mencetak kader yang berkarakter dan kritis.' },
    { id: 'tl-5', year: '2026', tag: 'TRANSFORMASI', title: 'Era Kolaborasi & Platform Terpadu', location: 'Kabupaten Banyumas', desc: 'Peluncuran platform digital terpadu, penguatan jurnalisme literasi, kemandirian kas dakwah, dan jejaring lebih dari 17 sekolah binaan.' },
  ];

  const rawStats = home?.stats;
  ...
  const latestArticles = [...articles].sort((a, b) => new Date(b.publishedAt || b.date) - new Date(a.publishedAt || a.date)).slice(0, 3);
  const upcomingEvents = events.filter(e => e.status === 'upcoming' || !e.status).slice(0, 3);

  const heroPhotoSafe = getSafeMediaUrl(hero.photoUrl);
  const aboutPhotoSafe = getSafeMediaUrl(about.photoUrl);

  return (
    <div style={{ overflowX: 'hidden' }}>
      {/* ═══════════════════════════════════════════════════ */}
      {/* 1. HERO SECTION (DYNAMIC CMS TAB 1)                 */}
      {/* ═══════════════════════════════════════════════════ */}
      <section style={{
        ...
        <div className="container" style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1, width: '100%' }}>
          <div style={{
            ...
            <div>
              <div className="animate-fade-in-up" style={{ marginBottom: '1.25rem' }}>
                <span className="badge badge-emerald" style={{ fontSize: '0.78rem', padding: '0.4rem 1rem', letterSpacing: '0.08em' }}>
              ...
              {hero.leadPhrase && (
                <p className="animate-fade-in-up delay-150" style={{
              ...
              <div className="animate-fade-in-up delay-300" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {/* Right: Editorial Documentary Framing (Photo & Archive Card) */}
            {heroPhotoSafe && (
          ...
          {/* ═══════════════════════════════════════════════════ */}
          {/* 2. STATS GRID (DYNAMIC CMS TAB 5)                   */}
          {/* ═══════════════════════════════════════════════════ */}
          <div style={{
            display: 'grid',
        ...
        <div className="container" style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{
            ...
            <div>
              ...
              {about.paragraph1 && (
                <p style={{ color: 'rgba(13,43,34,0.7)', lineHeight: 1.8, marginBottom: '1rem', fontSize: '0.96rem' }}>
                  {about.paragraph1}
                </p>
              )}

              {about.paragraph2 && (
                <p style={{ color: 'rgba(13,43,34,0.7)', lineHeight: 1.8, marginBottom: '1.75rem', fontSize: '0.96rem' }}>
                  {about.paragraph2}
                </p>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginBottom: '2rem' }}>
                {about.point1Title && (
                  <div style={{ background: 'white', padding: '1.25rem', borderRadius: 'var(--radius-lg)', border: '1px solid rgba(13,43,34,0.08)', boxShadow: 'var(--shadow-sm)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.4rem', color: 'var(--deep-pine)', fontWeight: 700 }}>
                ...
                {about.point2Title && (
                  <div style={{ background: 'white', padding: '1.25rem', borderRadius: 'var(--radius-lg)', border: '1px solid rgba(13,43,34,0.08)', boxShadow: 'var(--shadow-sm)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.4rem', color: 'var(--deep-pine)', fontWeight: 700 }}>
                      <CheckCircle2 size={18} style={{ color: 'var(--antique-brass)' }} />
              ...
              <div>
                ...
                {about.quoteText && (
                  <div style={{
                    ...
                    <p style={{ fontStyle: 'italic', color: 'var(--deep-pine)', fontSize: '0.94rem', lineHeight: 1.6, margin: '0 0 0.5rem' }}>
                      {about.quoteText}
                    </p>
                    {about.quoteAuthor && (
      ...
      {/* ═══════════════════════════════════════════════════ */}
      {/* 4. 5 PILAR DIVISI (DYNAMIC CMS TAB 3)               */}
      {/* ═══════════════════════════════════════════════════ */}
        ...
        <div className="container" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
          <div className="section-header">
            <span className="badge" style={{ background: 'rgba(16,185,129,0.15)', color: 'var(--emerald-light)', marginBottom: '0.75rem' }}>
              <ShieldCheck size={14} /> Struktur Gerakan
            </span>
          ...
          <div style={{
            ...
            {defaultDivisionPrograms.map((div, i) => {
              const IconComp = div.icon;
              return (
                <div key={div.id} className="card-editorial animate-fade-in-up" style={{
                ...
                >
                  <div>
                    <div style={{
                      ...
                      <IconComp size={22} style={{ color: 'var(--emerald-light)' }} />
                    </div>

                    <span className="badge badge-gold" style={{ fontSize: '0.68rem', padding: '0.15rem 0.6rem', marginBottom: '0.5rem' }}>
                      {div.division}
                    </span>
                    ...
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '1rem' }}>
                      <div style={{ fontSize: '0.74rem', color: 'var(--antique-brass)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.5rem' }}>
                        Program Kerja Unggulan:
                      </div>
                      <ul style={{ margin: 0, paddingLeft: '1.2rem', color: 'rgba(245,242,237,0.7)', fontSize: '0.82rem', lineHeight: 1.6 }}>
                        {div.programs.map((p, pIdx) => (
      ...
      {/* ═══════════════════════════════════════════════════ */}
      {/* 5. HISTORIA & LINIMASA (DYNAMIC CMS TAB 4)          */}
      {/* ═══════════════════════════════════════════════════ */}
      <section className="section" style={{ background: 'var(--warm-alabaster)', padding: '5.5rem 0' }}>
        <div className="container" style={{ maxWidth: 1000, margin: '0 auto', padding: '0 1.5rem' }}>
          <div className="section-header">
            <span className="badge badge-emerald" style={{ marginBottom: '0.75rem' }}>
              <Clock size={14} /> Jejak Perjalanan
            </span>
          ...
          <div style={{ position: 'relative', marginTop: '3rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              {timeline.map((item, idx) => (
                <div key={item.id || idx} className="animate-fade-in-up" style={{
                  display: 'flex',
                  gap: '1.5rem',
                  alignItems: 'flex-start',
                  position: 'relative',
      ...
      <section className="section" style={{ background: '#F8F6F0', padding: '5rem 0' }}>
        <div className="container" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
          <div className="section-header">
            ...
            <h2 style={{ color: 'var(--deep-pine)' }}>Literasi Dakwah Moderat</h2>
            <div className="section-divider" />
            <p style={{ color: 'rgba(13,43,34,0.6)' }}>Bacaan Islami berkualitas untuk memperluas wawasan dan memperkuat iman pemuda.</p>
          </div>
      ...
      <section className="section section-pine" style={{ padding: '5rem 0' }}>
        <div className="container" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
          <div className="section-header">
            ...
            <h2 style={{ color: 'var(--warm-alabaster)' }}>Kegiatan & Kajian ROKABA</h2>
            <div className="section-divider" />
            <p style={{ color: 'rgba(245,242,237,0.6)' }}>Ikuti berbagai kegiatan dakwah dan pengembangan diri bersama kader se-Banyumas.</p>
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: '2.5rem' }}>
            <Link to="/events" className="btn btn-primary">
              Lihat Seluruh Agenda Kegiatan <ChevronRight size={18} />
            </Link>
          </div>
        </div>
      </section>
      ...
      {/* ═══════════════════════════════════════════════════ */}
      {/* 8. FASILITAS & FITUR EKOSISTEM                       */}
      {/* ═══════════════════════════════════════════════════ */}
      <section className="section" style={{ padding: '5rem 0', background: 'var(--warm-alabaster)' }}>
        <div className="container" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
          <div className="section-header">
            <h2 style={{ color: 'var(--deep-pine)' }}>Layanan & Direktori Dakwah</h2>
            <div className="section-divider" />
            <p style={{ color: 'rgba(13,43,34,0.6)' }}>Berbagai fasilitas terpadu untuk mendukung syiar dan komunikasi antar sekolah.</p>
          </div>
          ...
          <div style={{
            ...
            {[
              { icon: MessageCircle, title: 'Konsultasi & Tanya Jawab', desc: 'Ruang tanya jawab keislaman dan problematika remaja bersama asatidz.', link: '/consultation', color: 'var(--emerald)' },
              { icon: Library, title: 'E-Library & Modul', desc: 'Unduh modul kaderisasi, slide materi kajian, dan kurikulum pembinaan gratis.', link: '/library', color: 'var(--antique-brass)' },
              { icon: School, title: 'Direktori ROHIS Sekolah', desc: 'Database sekolah anggota SMA/SMK/MA terdaftar se-Kabupaten Banyumas.', link: '/schools', color: '#7C3AED' },
            ].map((feature, i) => (
              ...
              >
                ...
                <p style={{ fontSize: '0.9rem', color: 'rgba(13,43,34,0.6)', lineHeight: 1.6 }}>{feature.desc}</p>
      ...
      {/* ═══════════════════════════════════════════════════ */}
      {/* 9. AJAKAN BERGABUNG / CTA (DYNAMIC CMS TAB 6)       */}
      {/* ═══════════════════════════════════════════════════ */}
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 680, margin: '0 auto' }}>
          <span className="badge badge-emerald" style={{ marginBottom: '1.25rem', padding: '0.35rem 0.9rem' }}>
